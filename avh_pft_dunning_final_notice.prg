 ...
 *001 07/23/22  Shruthi R			SR-92137:duplicate the avh_pft_dunning_statement_v2
 									 but only have the final notices
 									 and have this send separately to CBB.
 *002 01/31/23 Pavan M				SR-116990				
 *003 02/22/23 Pavan M               Corrected incorrect guarntor address	 
 ***********************************************************************
 
 ******************  END OF ALL MODCONTROL BLOCKS  ********************/
 
drop program avh_pft_dunning_final_notice go
create program avh_pft_dunning_final_notice
...
select into "nl:"
 ...
 plan b where ( b.submit_dt_tm between cnvtdatetime(s_dt)and cnvtdatetime(e_dt)) or( b.beg_effective_dt_tm between cnvtdatetime(s_dt)
    	                         and cnvtdatetime(e_dt));005
    	       ; b.submit_dt_tm between sysdate-1 and sysdate
 ...
 join pe where pe.pft_encntr_id = br.parent_entity_id
         ...
         /*  and  PE.DUNNING_LEVEL_CD IN ( 649883.00   ;Collections # 1
 ...
 join sc where sc.statement_cycle_id = pe.statement_cycle_id
           ...
           and sc.statement_cycle_cd in (2556436781.00     ;AV Hospital FPP Final Notice
 ...
 */
 		IF ( PE.DUNNING_LEVEL_CD IN ( 649883.00   ;Collections # 1
										, 649882.00   ;Collections # 2
